...
import (
	...
	"emperror.dev/errors"
	"github.com/DavidReque/go-food-delivery/internal/pkg/core/messaging/bus"
...
func NewRabbitmqBus(
	...
	// Build the rabbitmq configuration
	builder := configurations.NewRabbitMQConfigurationBuilder()
	if rabbitmqBuilderFunc != nil {
		rabbitmqBuilderFunc(builder)
	}

	rabbitmqConfiguration := builder.Build() // Build the rabbitmq configuration

	// Create the rabbitmq bus
	...
	for _, consumerConfiguration := range consumersConfigurationMap {
		...
		// Add the consumer to the message type consumers map
...
func (r *rabbitmqBus) ConnectConsumerHandler(
	...
	if consumersForType != nil {
		for _, c := range consumersForType {
			c.ConnectionHandler(consumerHandler)
		}
	} else {
