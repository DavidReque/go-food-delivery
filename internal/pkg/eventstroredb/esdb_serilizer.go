...
import (
	...
	appendResult "github.com/DavidReque/go-food-delivery/internal/pkg/es/models/append_result"
	readPosition "github.com/DavidReque/go-food-delivery/internal/pkg/es/models/stream_position/read_position"
	"github.com/DavidReque/go-food-delivery/internal/pkg/es/models/stream_position/truncatePosition"
	expectedStreamVersion "github.com/DavidReque/go-food-delivery/internal/pkg/es/models/stream_version"
	esErrors "github.com/DavidReque/go-food-delivery/internal/pkg/eventstroredb/errors"
...
func (e *EsdbSerializer) StreamEventToEventData(
	...
	id, err := gofrsUUID.FromString(streamEvent.EventID.String())
	if err != nil {
		return *new(esdb.EventData), err
	}
...
func (e *EsdbSerializer) ResolvedEventToStreamEvent(
	...
	// Convertir el evento deserializado a IDomainEvent
	domainEvent, ok := deserializedEvent.(domain.IDomainEvent)
	if !ok {
		return nil, errors.New("deserialized event does not implement IDomainEvent")
	}

	return &models.StreamEvent{
		EventID:  utils.ConvertSatoriUUIDToGoogleUUID(id),
		Event:    domainEvent,
...
func (e *EsdbSerializer) Deserialize(
	...
	// Convertir el payload a IDomainEvent
	domainEvent, ok := payload.(domain.IDomainEvent)
	if !ok {
		return nil, nil, errors.New("deserialized payload does not implement IDomainEvent")
	}

	return domainEvent, meta, nil
}
...
func (e *EsdbSerializer) DomainEventToStreamEvent(
	...
	if err != nil {
		// En caso de error, usar un UUID por defecto
		id = uuid.Nil
	}
