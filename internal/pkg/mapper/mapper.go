...
import (
	...
	defaultLogger "github.com/DavidReque/go-food-delivery/internal/pkg/logger/defaultlogger"
...
func MapSlice[TSrc any, TDst any](src []TSrc) ([]TDst, error) {
	dest := make([]TDst, len(src))
	var err error

	for i, item := range src {
		dest[i], err = Map[TDst, TSrc](item)
		if err != nil {
			return nil, err
		}
	}
